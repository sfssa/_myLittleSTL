...
声明为constexpr函数的意义是：如果其参数均为合适的编译期常量，
则对这个constexpr函数的调用就可用于期望常量表达式的场合 */
template <class T>
struct deque_buf_size
{
    static constexpr size_t value = sizeof(T) < 256 ? 4096 / sizeof(T) : 16;
};
/* deque和vector类似，但是并非是放在连续的内存中，而是放在多个非连续的
内存块中，通过一个map映射来得到某一块连续的内存 */
// deque 的迭代器设计
...
struct deque_iterator : public iterator<random_access_iterator_tag, T>
{
    ...
    map_pointer   node;   // 缓冲区所在节点，也就是map映射表的位置
...
/* 在头部和尾部就地构造元素时需要考虑是否越界的问题，对于头部插入需要考虑插入后是否超过了该连续内存的空间
，如果超过了就需要再分配一个map节点并添加映射；同时在尾部就地构造元素时也需要考虑是否超过了容量，超过了
容量需要重新开辟一个节点来装新构建的元素 */
// 在头部就地构建元素
template <class T>
template <class ...Args>
...
/* 例如，有一个数组4，5，6，7，8；每一个数字对应一块连续内存的首地址，现在需要弹出第5块内存
的首个元素，而此时第5块内存的元素数目为0，那么首个元素就变成了第6块地址的第一个元素，因此需要把
当前map指针移动到下一个位置也就是6，然后再去弹出元素 */
// 弹出头部元素
template <class T>
...
// 在 position 处插入元素
/* 插入元素时分为三类，一个插入在第一个节点前，一个插入最后一个节点后
最后是在中间位置插入 */
template <class T>
...
// 删除 position 处的元素
/* 这里由于用数组作为map，因此当删除某个节点时需要把整个数组进行覆盖式删除，需要判断是从开始节点到
删除节点的距离多还是删除节点到最后一个节点距离大，选择距离更小的来覆盖式删除 */
template <class T>
typename deque<T>::iterator
...
{
    /* 这里有一个|+1，个人认为可以多加几个缓冲区，避免一旦越界直接崩溃，增加程序的鲁棒性 */
    ...
    // 让 nstart 和 nfinish 都指向 map_ 最中央的区域，方便向头尾扩充，deque是一个双向的vector方便扩充
