...
#include "util.h"
/* 
这个头文件包含了 mystl 的基本算法，最好熟悉lambda表达式的用法会比较好理解 */
namespace mystl
{
    /* 
    如果max已经宏定义过，那么执行下面的代码，然后删除宏定义 */
#ifdef max
...
/* 这里的comp可以看作一种规则，支持lambda表达式，代表了如何判断大小 */
...
//这个函数用来把两个迭代器所指向的对象进行交换
template <class FIter1, class FIter2>
void iter_swap(FIter1 lhs, FIter2 rhs)
{
  mystl::swap(*lhs, *rhs);
}
// 下面的方法是把 [first, last)区间内的元素拷贝到 
...
unchecked_copy(Tp* first, Tp* last, Up* result)
{
  ...
  if (n != 0)
    std::memmove(result, first, n * sizeof(Up));//这里移动的是n个对象所占内存
  return result + n;
}
...
// 为 trivially_copy_assignable （是否有普通赋值运算符，trivially_copy是拷贝不变的意思）类型提供特化版本
template <class Tp, class Up>
typename std::enable_if<
  std::is_same<typename std::remove_const<Tp>::type, Up>::value &&
