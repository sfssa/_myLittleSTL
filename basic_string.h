...
// char_traits
/* 是一个与机器相关的 unsigned 整型类型，其大小足以保证存储内存中对象的大小
size_t的大小是由你生成的程序类型决定的，只是生成的程序类型与系统的类型有一定
关系。32bits的程序既可以在64bits的系统上运行，也可以在32bits的系统上运行。但
是64bits的程序只能在64bits的系统上运行。然而我们编译的程序一般是32bits的，因
此size_t的大小也就变成了4个字节 */
/* 下面的不同类是为了全特化和偏特化 */
...
struct char_traits
{
  ...
  {
    size_t len = 0;
    //*str != char_type(0)指的是没有到达结束符eof，函数返回字符串的长度
    for (; *str != char_type(0); ++str)
      ++len;
    return len;
  }
    /* 比较两个字符串，一个字符一个字符比较，当某一个字符出现结果后后面的字符不在比较 */
  ...
  {
    MYSTL_DEBUG(src + n <= dst || dst + n <= src);//这里是为了避免处于同一块内存发生覆盖
    char_type* r = dst;
    for (; n != 0; --n, ++dst, ++src)
      *dst = *src;
    return r;
...
// Partialized. char_traits<char>（局部的）
template <> 
struct char_traits<char>
{
  ...
  {
    MYSTL_DEBUG(src + n <= dst || dst + n <= src);//复制时候避免覆盖
...
/* wchar_t 是C/C++的字符类型，一种扩展的存储方式,char是8位字符类型，最多能包含256
种字符，许多的外文字符集所包含的字符数目超过256个，char型不能表示.wchar_t类型，也
称为双字节类型，或宽字符类型,它们的每一个文字都占据两个字节 */
// Partialized. char_traits<wchar_t>
//下面的函数是针对wchar_t来设计的，目的和上述的char相同
template <>
...
/* char16_t是用于16位宽字符的无符号整数类型
任何给定平台上，类型 char16_t 的宽度可能大于 16 位，但存
储于 char16_t 类型对象中的实际值将始终拥有 16 位宽度。 */
template <>
...
#define STRING_INIT_SIZE 32//初始化空间32位
// 模板类 basic_string
...
public:
    // 末尾位置的值，例:
    // if (str.find('a') != string::npos) { /* do something */ }，字符串结束符
...
public:
    // 迭代器相关操作，主要是返回迭代器的起始位置和结束位置，包括反转的迭代器
    ...
    /* 这里的begin指向第一个元素，而end指向的是最后一个元素的下一个位置 */
    reference front() 
    { 
        MYSTL_DEBUG(!empty());
        return *begin(); 
