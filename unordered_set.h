...
public:
    ...
    /* key代表键值，hash是哈希函数，keyequal是等比函数的函数对象，通过==来
    判断两个key值是否相等，想使用自定义的键类型必须实现哈希函数和等比函数 */
    // 构造、复制、移动函数
    unordered_set()
        :ht_(100, Hash(), KeyEqual())
    {
    }
    //显式声明构造函数，只能显式的进行类型转换
    ...
    /*  emplace() 可以在适当的位置直接构造新元素
    emplace_hint() 调用使用一个迭代器作为指示符，指向先前 emplace() 调用返回的 pair 对象 */
    template <class ...Args>
    ...
    //装载因子，也就是现在哈希表已经用了的内存比上总分配内存
    float     max_load_factor()        const noexcept { return ht_.max_load_factor(); }
    void      max_load_factor(float ml)               { ht_.max_load_factor(ml); }
    /* reserver函数用来给vector预分配存储区大小，即capacity的值 ，但是没有给这段内存进行初始
    化。reserve 的参数n是推荐预分配内存的大小，实际分配的可能等于或大于这个值，即n大于capacity的
    值，就会reallocate内存 capacity的值会大于或者等于n 。这样，当ector调用push_back函数使得size 
    超过原来的默认分配的capacity值时 避免了内存重分配开销。需要注意的是：reserve 函数分配出来的内
    存空间，只是表示vector可以利用这部分内存，但vector不能有效地访问这些内存空间，访问的时候就会出现
    越界现象，导致程序崩溃 */
    void      reserve(size_type count)                { ht_.reserve(count); }
    /* rehash问题, hash需要将key通过hash函数映射成size_t,再存储到一块预先申请好的连续空间上,因
    此当这些空间不够用了以后就需要重新分配空间并对原有元素进行重新计算&分配.这是很耗时间的. 如果能预
    估出所存元素的最大个数,可以用rehash或构造函数一次申请足够的空间,这样就可以避免rehash. */
...
public:
    /* equal_range_unique用来在指定范围内查找等于目标值的所有元素 */
...
public:
    ...
    template <class InputIterator>
    unordered_multiset(InputIterator first, InputIterator last,const size_type bucket_count = 100,const Hash& hash = Hash(),const KeyEqual& equal = KeyEqual())
        : ht_(mystl::max(bucket_count, static_cast<size_type>(mystl::distance(first, last))), hash, equal)
    {
        for (; first != last; ++first)
